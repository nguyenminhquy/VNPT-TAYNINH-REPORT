...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const REQUIRED_KEYS = ['mbb', 'fbb', 'mytv', 'mll', 'ispeed', '5s', 'xlsc', 'appendix', 'omc_tam', 'omc_nhi'];
    ...
    try {
      console.log(`[export-word-monthly] Proxying to: ${backendUrl}/export-word-monthly`);
      const renderRes = await fetch(`${backendUrl}/export-word-monthly`, {
        method: 'POST',
      ...
      const contentDisposition = renderRes.headers.get('content-disposition') ||
        'attachment; filename="Bao_cao_VNPT_thang.docx"';
